...
bool gs_boot()
{
  ...
  gs_stage = 2;
  gs_port = 0xF3;
  // После сброса ПЗУ GS сначала проверяет память и только потом берёт
  // команды: на карте 1 МБ ~4,3 с, 512 КБ — ~2 с (ПЗУ 1.04/1.05a в модели
  // tests/gsemu.py). Ролик, запущенный сразу после включения или сброса Evo
  // (курсор WC уже стоит на нём), уходил со звуком на FT812. Карта на шине
  // есть — ждём принятия #F3 до ~8 с: 200 раз по два тика WC.
  for (i = 0; !gs_wait(1, 0, 2); )
