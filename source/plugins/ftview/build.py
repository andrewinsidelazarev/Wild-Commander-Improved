...
def build(out):
    ...
    banks = {}
    payloads = []
    for bank, number in (('image', 0), ('video', 9)):
        (out/bank).mkdir(exist_ok=True)
        prefix = bank + '/'
        define = ['-DFTVIEW_VIDEO_BANK'] if number else []
        ...
        for name, addr in {'_fs_buf':0xC000, '_ret_sp':0xBFE0, '_ret_ix':0xBFE2,
                           ...
                           '_wc_video_bank':0xBF80, **({'_hud_cmd':0xBBD0} if number else {})}.items():
            assert symbols[name] == addr, (bank,name,symbols[name])
        for name,(addr,size) in layout.items():
            if not size:
                continue
            if name in ('_CODE','_HOME'):
                # Видеобанк держит шаблон HUD AVI в #BBD0..#BCFF (hud_cmd).
                limit = 0xBBD0 if number else 0xBD00
        ...
        assert ih.minaddr() == 0x8000 and ih.maxaddr() == 0xBFC5
    ...
    assert payloads[0][0x3F80:0x3FA5] == payloads[1][0x3F80:0x3FA5]
    assert payloads[0][0x3FA7:0x3FC6] == payloads[1][0x3FA7:0x3FC6]
    # Вход #8003 ведёт в far_entry своего банка.
    assert payloads[0][3] == payloads[1][3] == 0xC3
    run([compiler,*common,*inc,'-DN_BLK0=32','-DN_BLK9=32',
    ...
    assert len(header) == 512 and header[34] == 10
    assert header[36:42] == bytes([0,32,9,32,1,0])
    ...
    report = dict(banks=banks, map_sha256=hashlib.sha256((out/'ftview.map').read_bytes()).hexdigest(), bytes=len(binary),sha256=hashlib.sha256(binary).hexdigest(),areas=layout,
